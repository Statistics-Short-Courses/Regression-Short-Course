...
Linear regression models how a *response variable* (Y) depends on one
or more *predictors* (X).

$$Y_i = \beta_0 + \beta_1 X_i + \varepsilon_i, \quad \varepsilon_i \sim N(0, \sigma^2)$$

This equation tells us that we can model the the '$i^{th}$' observation $Y_i$, by taking a 'linear combination' of the $i^{th}$ observation of X - that is $X_i$ multiplied or added to some constant values $\beta0, \beta$ - plus some random error $\varepsilon_i$ which we assume to be normally distributed. 
...
Lets start with our predictor variable, $X$. Say we have records for 10 employees at a company, including their time working at the company. These values, in years are:
```{r}
