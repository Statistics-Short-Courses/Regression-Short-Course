...
//| echo: false
//| width: 700
//| fig.cap: "The probability density of observations ($Y$) under a simple linear model."
 xValues = d3.range(-10,10.001, 0.3)
...
//| echo: false
//| width: 700
//| fig-cap: "Cross-sections of the simple linear model showing Normal error curves at selected X values."

xSampleValues = d3.range(-8, 8.1, 2)
...
densityCurveData = xSampleValues.flatMap(xVal => {
  ...
  const leftSide = ySectionValues.slice().reverse().map(y => {
    const density = normalDensity(y, mu, sigma_2);
    const width = (density / peakDensity) * widthScale;
    return {x: xVal - width, y, group: xVal};
  });

  const closedPath = [...rightSide, ...leftSide, rightSide[0]];
  return closedPath.map((point, index) => ({...point, order: index}));
});

crossSectionTrendLine = xValues.map(x => ({
  x,
  y: b0_2 + b1_2 * x
}))
