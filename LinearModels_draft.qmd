...
# Understanding simple linear models

##  Linear relationships between two variables

Regression analysis models how a *response variable* ($Y$) depends on one
or more *predictors* (X). A simple linear regression assumes that there is an approximately linear relationship between a observations of a predictor $x$ and the outcome $y$. That is, that we can express $y$ as $x$  multiplied by and added to some constant values ($\beta$). For example, with one predictor variable we have a 'simple linear regression':
